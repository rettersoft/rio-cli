...
export class Deployment {
  static isChanged(deploymentSummary: IPreDeploymentContext) {
    for (const item of [
      ...
      ...deploymentSummary.classDeploymentsSummary.classDeploymentsSummary
        .createdItems,
      ...deploymentSummary.classDeploymentsSummary.classDeploymentsSummary
        .deletedItems,
    ]) {
  ...
  static async deploy(
          ...
          case DeploymentObjectItemType.DEPENDENCY:
            switch (item.status) {
              ...
              case DeploymentObjectItemStatus.EDITED:
                if (!item.newContent)
                  throw new Error(
                    "Dependency new content not found" + item.path
                  );
                if (!item.hash)
                  throw new Error("Dependency hash not found" + item.path);
    ...
    for (const className of Object.keys(
      ...
      if (
        ...
        currentClassDeploymentItem.status === DeploymentObjectItemStatus.DELETED
      )
        continue;
      ...
      for (const item of changedFileDeployments) {
        ...
        switch (item.status) {
          case DeploymentObjectItemStatus.CREATED:
            preparedData.push({
              status: "ADDED",
              content: gzipSync(Buffer.from(item.newContent!)).toString(
                "base64"
              ),
              name: classFileName,
            });
          ...
          case DeploymentObjectItemStatus.NONE:
            ...
            if (force) {
              preparedData.push({
                status: "EDITED",
                name: classFileName,
                content: gzipSync(Buffer.from(item.oldContent!)).toString(
                  "base64"
                ),
              });
            }
  ...
  static getModelDeploymentsContext(
    localModels: IProjectModels,
    remoteModels: IProjectModels
  ): IDeploymentSummary {
    /**
     * IGNORE LOCAL FILES
     */
    localModels = Object.keys(localModels).reduce<IProjectModels>(
      (acc, localModelName) => {
        if (
          !Ignore.isIgnored(
            path.relative(
              ...
              path.join(
                ...
                localModelName + PROJECT_MODEL_FILE_EXTENSION
              )
            )
          )
        ) {
     ...
     * IGNORE REMOTE FILES
     */
    remoteModels = Object.keys(remoteModels).reduce<IProjectModels>(
      (acc, remoteModelName) => {
        if (
          !Ignore.isIgnored(
            path.relative(
              ...
              path.join(
                PROJECT_MODELS_FOLDER,
                remoteModelName + PROJECT_MODEL_FILE_EXTENSION
              )
            )
          )
        ) {
    ...
    const created = Object.keys(localModels).reduce<IDeploymentOperationItem[]>(
      (acc, modelName) => {
    ...
    Object.keys(localModels).map((modelName) => {
      if (remoteModels[modelName]) {
        if (
          JSON.stringify(localModels[modelName]) !==
          JSON.stringify(remoteModels[modelName])
        ) {
          edit.push({
            path: modelName,
            type: DeploymentObjectItemType.MODEL,
  ...
  static getClassDeploymentsContext(
    ...
    Object.keys(localClasses).forEach((className) => {
      ...
      Object.keys(localClasses[className]).forEach((fileName) => {
        ...
        if (
          !remoteClasses[className] ||
          remoteClasses[className][fileName] === undefined
        ) {
          ...
          if (
            localClasses[className][fileName] !==
            remoteClasses[className][fileName]
          ) {
  ...
  static getDependencyDeploymentsContext(
    localDependencies: IDependencyContent[],
    remoteDependencies: IRemoteDependencyContent[]
  ): IDeploymentSummary {
    ...
    remoteDependencies.forEach((rd) => {
      if (
        !localDependencies.find((ld) => ld.dependencyName === rd.dependencyName)
      ) {
        deletedItems.push({
    ...
    localDependencies.forEach((ld) => {
      const remoteModel = remoteDependencies.find(
        (rd) => rd.dependencyName === ld.dependencyName
      );
      if (!remoteModel) {
        createdItems.push({
